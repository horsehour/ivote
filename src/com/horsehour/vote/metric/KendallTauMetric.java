 ...
 * @author mao
 */
public class KendallTauMetric<T extends Comparable<T>> extends RankingMetric<T> {
	...
	public double computeByScore(ScoredItems<T> scores) {
		/*
		 * Expected random tie breaking: If two alternatives are ranked equally
		 * then that is half a mistake
		 * 
		...
		for (int i = 0; i < keys.size(); i++) {
			for (int j = i + 1; j < keys.size(); j++) {
				int compare = new Double(scores.get(keys.get(i)).doubleValue())
						.compareTo(scores.get(keys.get(j)).doubleValue());
				if (compare < 0)
					mistakes += 1;
				else if (compare == 0)
					mistakes += 0.5;
