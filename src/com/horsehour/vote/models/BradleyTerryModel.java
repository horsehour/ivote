...
public class BradleyTerryModel extends PairwiseDiscreteChoiceEstimator<GumbelNoiseModel<?>> {
	...
	public BradleyTerryModel() {
		ConvergenceChecker<PointValuePair> checker = new SimplePointChecker<PointValuePair>(1e-5, 1e-8);
	...
	public double[] getParameters(double[][] wins) {
		...
		try {
			result = optim.optimize(func, grad, GoalType.MINIMIZE, init, new MaxIter(500), new MaxEval(500));
			cgUses.incrementAndGet();
		} catch (MathIllegalStateException e) {
			/*
			 * Had problems with returning 0. Initialize with some small random
			 * numbers...
			 */
			...
			// TODO: Default value of 50 iterations in BracketFinder here seems
			// to cause problems
			result = backup.optimize(func, GoalType.MINIMIZE, init, new MaxEval(5000), new MaxIter(1000));
	...
	@Override
	public <T> GumbelNoiseModel<T> fitModel(Profile<T> profile, boolean useAllPairs) {
		List<T> ordering = Arrays.asList(profile.getSortedItems());

		double[][] wins = useAllPairs ? addAllPairs(profile, ordering) : addAdjacentPairs(profile, ordering);

		double[] strengthParams = getParameters(wins);

		return new GumbelNoiseModel<T>(ordering, strengthParams);
	}
}
