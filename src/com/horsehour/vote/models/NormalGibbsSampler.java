...
import com.horsehour.vote.stat.TruncatedNormal;
import com.horsehour.vote.stat.TruncatedNormalQuick;
...
class NormalGibbsSampler implements Callable<NormalMoments> {
	...
	NormalGibbsSampler(RealVector delta, RealVector variance, int[] ranking, int samples, boolean storeVar,
		...
		for (int i = 0; i < sigmas.length; i++)
			sigmas[i] = Math.sqrt(sigmas[i]); // Take sqrts here for faster
												// sampling later
	...
	public NormalMoments call() {
		/*
		 * Initialize sampler with consistent random x_t Draw uniforms and sort
		 * according to values in delta
		 * 
		...
		Arrays.sort(rands);
		/*
		 * Put random values from greatest to least, 0..c-1 based on values in
		 * delta
		 */
		for (int i = 0; i < c; i++)
	...
	private void sample(int i, double mu, double sigma) {
		/*
		 * One step of the gibbs sampling Truncated normal sample of the value
		 * at index i
		 * 
