...
public abstract class MinFlipMistakesRule<T> extends OptimizedPositionalRule<T> {
		...
		for (Profile<T> profile : preferences) {
			...
			for (int i = 0; i < choices.length; i++) {
				for (int j = i + 1; j < choices.length; j++) {
					// score of i should be greater than score if j, if not then
					// we have a flip
			...
			/*
			 * Normalize so that the sum of the minMargins can be no greater
			 * than 1 so that the flips always dominate
			 */
	...
	protected void initStep(int p, Profile<T> profile) throws IloException {
	}

	protected void processPair(int p, Profile<T> profile, IloIntVar flip, int i, int j,
			IloLinearNumExpr total_i, IloLinearNumExpr total_j) throws IloException {
	}
	...
	protected void solve(IloNumExpr subObj) throws IloException {
		cp.addObjective(IloObjectiveSense.Minimize, subObj != null ? cp.sum(cp.sum(flipArr), subObj) : cp.sum(flipArr));
		cp.solve();
	...
	public static class MaxSumMinEachMargin<T> extends MinFlipMistakesRule<T> {
		...
		protected void initStep(int p, Profile<T> profile) throws IloException {
			k = 0;
			currentMaxPossibleMargin = normalization * profile.getNumVote();
		}

		@Override
		protected void processPair(int p, Profile<T> profile, IloIntVar flip, int i, int j,
				IloLinearNumExpr total_i, IloLinearNumExpr total_j) throws IloException {
			IloNumExpr condConstr = cp.sum(cp.prod(flip, currentMaxPossibleMargin), cp.diff(total_i, total_j));

			cp.addGe(marginMinsNorm[k], cp.prod(-1.0 / currentMaxPossibleMargin / preferences.size(), condConstr));
	...
	public static class MaxMinFirstLastMargin<T> extends MinFlipMistakesRule<T> {
		...
		@Override
		protected void initStep(int p, Profile<T> profile) throws IloException {
			currentMaxPossibleMinMargin = normalization / (numCandidates - 1) * profile.getNumVote();
		}
		...
		protected void processPair(int p, Profile<T> profile, IloIntVar flip, int i, int j,
			...
			if (i == 0 && j == numCandidates - 1) {
				...
				IloNumExpr condConstr = cp.sum(cp.prod(flip, currentMaxPossibleMinMargin), cp.diff(total_i, total_j));
	...
	public static class MaxOverallMinMargin<T> extends MinFlipMistakesRule<T> {
		...
		protected void processPair(int p, Profile<T> profile, IloIntVar flip, int i, int j,
			...
			IloNumExpr condConstr = cp.sum(cp.prod(flip, currentMaxPossibleMinMargin), margin);
			cp.addLe(currentMinMargin, condConstr);
		}
	...
	public static class MaxSumMinMargin<T> extends MinFlipMistakesRule<T> {
		...
		@Override
		protected void completeStep(int p) throws IloException {
			minMarginsNorm[p] = cp.prod(currentMinMargin, -1.0 / currentMaxPossibleMinMargin / preferences.size());
		}
	}
}
