...
/**
 * Multilayer perceptron neural network. An MLP consists of several layers of
 * nodes, interconnected through weighted acyclic arcs from each preceding layer
 * to the following, without lateral or feedback connections. Each node
 * calculates a transformed weighted linear combination of its inputs (output
 * activations from the preceding layer), with one of the weights acting as a
 * trainable bias connected to a constant input. The transformation, called
 * activation function, is a bounded non-decreasing (non-linear) function, such
 * as the sigmoid functions (ranges from 0 to 1). Another popular activation
 * function is hyperbolic tangent which is actually equivalent to the sigmoid
 * function in shape but ranges from -1 to 1. More specialized activation
 ...
 * function as activation function, the single-layer network is identical to the
 ...
 * multi-layer perceptron with just one hidden layer. This result holds only for
 * restricted classes of activation functions, which are extremely complex and
 * NOT smooth for subtle mathematical reasons. On the other hand, smoothness is
 * important for gradient descent learning. Besides, the proof is not
 * constructive regarding the number of neurons required or the settings of the
 * weights. Therefore, complex systems will have more layers of neurons with
 * some having increased layers of input neurons and output neurons in practice.
 * <p>
 ...
 * order to reduce the value of the error function by some small amount. For
 * this reason, back-propagation can only be applied on networks with
 ...
 * that the network converges to a local minimum of the error function fast
 * enough, without producing oscillations. One way to avoid oscillation at large
 * &eta;, is to make the change in weight dependent on the past weight change by
 * adding a momentum term.
 * <p>
 * Although the back-propagation algorithm may performs gradient descent on the
 * total error of all instances in a batch way, the learning rule is often
 * applied to each instance separately in an online way or stochastic way. There
 * exists empirical indication that the stochastic way results in faster
 * convergence.
 * <p>
 ...
 * naturally in a Bayesian framework, where the regularization can be performed
 * by selecting a larger prior probability over simpler models; but also in
 * statistical learning theory, where the goal is to minimize over the
 * "empirical risk" and the "structural risk".
 * <p>
 * For neural networks, the input patterns usually should be
 * scaled/standardized. Commonly, each input variable is scaled into interval
 * [0, 1] or to have mean 0 and standard deviation 1.
 * <p>
 ...
 * <li>a two-class cross-entropy penalty function and a logistic activation
 * function.
 * <li>a multi-class cross-entropy penalty function and a softmax activation
 * function.
 * </ul>
 * By assigning a softmax activation function on the output layer of the neural
 * network for categorical target variables, the outputs can be interpreted as
 * posterior probabilities, which are very useful.
 * 
 * @author Haifeng Li
 */
public class VoteNetwork implements OnlineClassifier<double[]>, SoftClassifier<double[]> {
	private static final Logger logger = LoggerFactory.getLogger(VoteNetwork.class);
	...
	public static class Trainer extends ClassifierTrainer<double[]> {
		...
		public VoteNetwork train(double[][] x, int[][] y) {
			VoteNetwork net = new VoteNetwork(errorFunction, activationFunction, numUnits);
	 ...
	 *            the number of units in each layer.
	 */
	public VoteNetwork(ErrorFunction error, int... numUnits) {
		this(error, natural(error, numUnits[numUnits.length - 1]), numUnits);
	}
	 ...
	 *            the number of units in each layer.
	 */
	public VoteNetwork(ErrorFunction error, ActivationFunction activation, int... numUnits) {
		int numLayers = numUnits.length;
		if (numLayers < 2) {
			throw new IllegalArgumentException("Invalid number of layers: " + numLayers);
		}
	...
	@Override
	public VoteNetwork clone() {
		VoteNetwork copycat = new VoteNetwork();
