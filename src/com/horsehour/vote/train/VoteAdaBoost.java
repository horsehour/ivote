...
/*******************************************************************************
 * Copyright (c) 2010 Haifeng Li Licensed under the Apache License, Version 2.0
 ...
 * <p>
 * AdaBoost calls a weak classifier repeatedly in a series of rounds from total
 * T classifiers. For each call a distribution of weights is updated that
 * indicates the importance of examples in the data set for the classification.
 * On each round, the weights of each incorrectly classified example are
 * increased (or alternatively, the weights of each correctly classified example
 * are decreased), so that the new classifier focuses more on those examples.
 * <p>
 * The basic AdaBoost algorithm is only for binary classification problem. For
 * multi-class classification, a common approach is reducing the multi-class
 * classification problem to multiple two-class problems. This implementation is
 * a multi-class AdaBoost without such reductions.
 * <h2>References</h2>
 * <ol>
 * <li>Yoav Freund, Robert E. Schapire. A Decision-Theoretic Generalization of
 * on-Line Learning and an Application to Boosting, 1995.</li>
 * <li>Ji Zhu, Hui Zhou, Saharon Rosset and Trevor Hastie. Multi-class Adaboost,
 * 2009.</li>
 * </ol>
 * 
 * @author Haifeng Li
...
public class VoteAdaBoost implements SoftClassifier<double[]> {
	 ...
	 * individual variable over all trees in the forest gives a fast variable
	...
	public static class Trainer extends ClassifierTrainer<double[]> {
		...
		/**
		 * Default constructor of 500 trees and maximal 2 leaf nodes in the
		 * tree.
		 */
		public Trainer() {
		...
		@Override
		public VoteAdaBoost train(double[][] x, int[] y) {
			return new VoteAdaBoost(null, x, y, ntrees, maxNodes);
		}
	}
	 ...
	 *            the number of trees.
	 */
	public VoteAdaBoost(double[][] x, int[] y, int ntrees) {
	 ...
	 */
	public VoteAdaBoost(double[][] x, int[] y, int ntrees, int maxNodes) {
	 ...
	 *            the number of trees.
	 */
	public VoteAdaBoost(Attribute[] attributes, double[][] x, int[] y, int ntrees) {
	 ...
	 *            the maximum number of leaf nodes in the trees.
	 */
	public VoteAdaBoost(Attribute[] attributes, double[][] x, int[] y, int ntrees, int maxNodes) {
		...
		double b = Math.log(k - 1); // the bias to tree weight in case of
									// multi-class.
		int failures = 0; // the number of weak classifiers less accurate than
							// guess.
		...
		for (int t = 0; t < ntrees; t++) {
			...
			trees[t] = new DecisionTree(attributes, x, y, maxNodes, 1, x[0].length, DecisionTree.SplitRule.GINI,
					samples, order);
	 ...
	 * decreases for each individual variable over all trees in the forest gives
	 * a simple measure of variable importance.
	 *
	...
	/**
	 * Trims the tree model set to a smaller size in case of over-fitting. Or if
	 * extra decision trees in the model don't improve the performance, we may
	 * remove them to reduce the model size and also improve the speed of
	 * prediction.
	 * 
	 * @param ntrees
