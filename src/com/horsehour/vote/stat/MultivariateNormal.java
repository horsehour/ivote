...
public class MultivariateNormal {
	...
	static {
		try {
			LibraryReplicator<MvnPackGenz> repl = new LibraryReplicator<MvnPackGenz>(MvnPackGenz.class.getClassLoader().getResource(
			        MvnPackGenz.MVNPACK_SO), MvnPackGenz.class, 1); // FIXME: concurrency issues
	...
	public MultivariateNormal(int maxPtsMultiplier, double cdf_abseps, double cdf_releps, double exp_abseps, double exp_releps, boolean safe) {
	...
	public MultivariateNormal() {
		this(default_maxpts_multiplier, cdf_default_abseps, cdf_default_releps, exp_default_abseps, exp_default_releps, true);
	}
	...
	public static class ExpResult extends CDFResult {
		...
		public ExpResult(double cdf, double cdfError, double[] expValues, double[] expErrors, boolean converged) {
	...
	public static class EX2Result extends ExpResult {
		...
		public EX2Result(double cdf, double cdfError, double[] expValues, double[] expErrors, double[] eX2Values, double[] eX2Errors,
		        boolean converged) {
	...
	static CDFResult cdf(RealVector mean, RealMatrix sigma, double[] lower, double[] upper, int maxPts, double abseps, double releps){
		...
		lib.mvndst_(new IntByReference(n), adjLower, adjUpper, infin, correl, maxpts, abseps_ref, releps_ref, error, value, inform);

		int exitCode = inform.getValue();
		if (exitCode == 2)
			throw new RuntimeException("Dimension error for MVN");
	...
	static ExpResult exp(RealVector mean, RealMatrix sigma, double[] lower, double[] upper, int maxPts, double abseps, double releps){
		// Copy bounds arrays because we modify them
		double[] adjLower = lower.clone();
		double[] adjUpper = upper.clone();
		int n = checkErrors(mean, sigma, adjLower, adjUpper);
		...
		lib.mvnexp_(new IntByReference(n), adjLower, adjUpper, infin, correl, maxpts, abseps_ref, releps_ref, errors, values, inform);
		...
		/*
		 * Rescale the expected values and errors very important since the
		 * computation is on variance 1 normal!
		 */
		for (int i = 0; i < n; i++) {
	...
	static EX2Result eX2(RealVector mean, RealMatrix sigma, double[] lower, double[] upper, int maxPts, double abseps, double releps){
		// Copy bounds arrays because we modify them
		double[] adjLower = lower.clone();
		double[] adjUpper = upper.clone();
		int n = checkErrors(mean, sigma, adjLower, adjUpper);
		...
		lib.mvnxpp_(new IntByReference(n), adjLower, adjUpper, infin, correl, maxpts, abseps_ref, releps_ref, errors, values, inform);
		...
		/*
		 * Rescaling The order of operations here is important because we use
		 * the first moment to compute the second
		 */
		for (int i = 0; i < n; i++) {
			...
			double mu_sq = mu * mu;
			/*
			 * Y = sX + u E[Y^2] = E[s^2X^2 + 2suX + u^2] Don't touch the first
			 * moments yet TODO The 2nd moment error calculation is sketchy...
			 */
			eX2Result[i] = var_i * eX2Result[i] + 2 * sds[i] * mu * expResult[i] + mu_sq;
			eX2Errors[i] = var_i * eX2Errors[i] + 2 * sds[i] * mu * expErrors[i];
			/*
			 * E[Y] = E[sX + u] Now we can modify them
			 */
	...
	private static int checkErrors(RealVector mean, RealMatrix sigma, double[] lower, double[] upper){
	...
	private static double[] getCorrelAdjustLimits(RealVector mean, RealMatrix sigma, double[] lower, double[] upper, double[] sd){
		int n = mean.getDimension();
