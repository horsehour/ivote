...
/**
 * Class to make a non-reentrant native library threadsafe by copying it and
 * maintaining a queue of locked objects
 * 
 * @author mao
 *
 */
public class LibraryReplicator<C> {
	...
	public LibraryReplicator(URL libraryResource, Class<C> interfaceClass, int copies) throws IOException {
		...
		for (int i = 0; i < copies; i++) {
			...
			Files.copy(origFile.toPath(), copy.toPath(), StandardCopyOption.REPLACE_EXISTING);
		...
		proxiedInterface = (C) Proxy.newProxyInstance(interfaceClass.getClassLoader(), new Class[] { interfaceClass },
	...
	/*
	 * Invocation handler that uses the queue to grab locks and maintain thread
	 * safety.
	 */
	private class BlockingInvocationHandler implements InvocationHandler {
		...
		public Object invoke(Object proxy, Method method, Object[] args) throws Exception {
			...
			try {
				...
				// Return the library to the queue, even if there is an
				// exception
